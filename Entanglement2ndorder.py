...
func23 = lambda  w1 : 4*math.pi*w1**2*(np.exp(-((w1-wa)**2/(2*a[l]**2))-1j*T[i]*wa)*np.sqrt(2/math.pi)*np.sqrt(w1)*(-np.exp(1j*T[i]*wa)*\
                                                                                                                    gamma+np.exp(1j*T[i]*w1)*(gamma*math.cosh(gamma*T[i])-1j*(w1-wa)*math.sinh(gamma*T[i])))/((gamma**2+(w1-wa)**2))) 

func31 = lambda  w1 : 4*math.pi*w1**2*(np.exp(-T[i]*(gamma+1j*(w1-wa))-(w1-wa)**2/(2*a[l]**2))*(-1+np.exp(T[i]*(gamma+1j*(w1-wa))))*\
                                       np.sqrt(w1))/(np.sqrt(2*math.pi)*(gamma+1j*(w1-wa))) 
...
for l in range(len(a)):
  ...
  plt.plot(T, Entanglement, label="N =" +str(np.real(round(NumberOfPhotons[l],4))))

plt.xlabel("Time")
plt.ylabel("Entanglement")
plt.legend(loc='upper center', bbox_to_anchor=(1.2, 1), shadow=True, ncol=1)
plt.savefig("SecondOrderEntanglement.png", format = "png", dpi = 150, bbox_inches = "tight")
